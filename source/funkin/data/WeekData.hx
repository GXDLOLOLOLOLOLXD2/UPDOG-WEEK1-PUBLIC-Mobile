...
class WeekData
{
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		if (mobile.backend.AssetUtils.assetExists(modsListPath)) // FileSystem.exists
		{
			...
			for (i in 0...stuff.length)
			{
				...
				else // Sort mod loading order based on modsList.txt file
				{
					...
					if (mobile.backend.AssetUtils.isAssetDirectory(path) // sys.FileSystem.isDirectory
		...
		for (i in 0...directories.length)
		{
			...
			if (mobile.backend.AssetUtils.assetExists(directory))//FileSystem.exists
			{
				var listOfWeeks:Array<String> = CoolUtil.coolTextFile(directory + 'weekList.txt');
				for (daWeek in listOfWeeks)
				{
					var path:String = directory + daWeek + '.json';
					if (mobile.backend.AssetUtils.assetExists(path)) // sys.FileSystem.exists
					{
